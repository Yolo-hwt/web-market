            ...
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  :key="attrValue.id"
                  @click="
                    changeAttrActive(attrValue, saleAttr.spuSaleAttrValueList)
                  "
                >
...
export default {
  ...
  computed: {
    ...mapGetters({
      categoryView: "categoryViewGetter",
      skuInfo: "skuInfoGetter",
      spuSaleAttrList: "spuSaleAttrListGetter",
    }),
    spuSaleCheckedAttr() {
      let checkedAttr = [];
      this.spuSaleAttrList.forEach((item) => {
        item.spuSaleAttrValueList.forEach((attr) => {
          if (attr.isChecked === "1") {
            checkedAttr.push(attr);
          }
        });
      });
      return checkedAttr;
    ...
    async addShopcar() {
      ...
      try {
        ...
        sessionStorage.setItem(
          "SKUATTRS",
          JSON.stringify(this.spuSaleCheckedAttr)
        );
      } catch (error) {
        alert(error.message);
      }
