  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="(good, index) in detailArrayList"
          :key="good.skuId"
        >
          <li>
            <img
          ...
          <li>
            <h3>￥{{ good.orderPrice * good.skuNum }}.00</h3>
          </li>
          <li>X{{ good.skuNum }}</li>
          <li>有货</li>
        </ul>
...
export default {
  ...
  computed: {
    ...mapGetters({
      addressInfo: "userAddressGetter",
      orderInfo: "ordersInfoGetter",
    }),
    detailArrayList() {
    ...
    totalOrderInfo() {
      let price = 0;
      let num = 0;
      this.detailArrayList.forEach((order) => {
        price += order.orderPrice * order.skuNum;
        num += order.skuNum;
      });
      return { price, num };
    },
  ...
  methods: {
    async getData() {
      try {
        ...
        await this.$store.dispatch("getOrdersInfo");
      } catch (error) {
    ...
    changeDefault(address) {
      this.addressInfo.forEach((item) => {
        ...
        if (
          ...
          address.phoneNum === item.phoneNum
        ) {
          item.isDefault = 1;
        }
      });
    ...
    async submitOrder() {
      ...
      const { consignee, phoneNum, fullAddress } = this.checkedAddress;
      const comment = this.message;
      const data = {
        consignee: consignee, //收件人
        consigneeTel: phoneNum, //手机号
        deliveryAddress: fullAddress, //地址
        paymentWay: "ONLINE", //付款方式
        orderComment: comment, //留言
