  ...
  <div class="pay-main">
    <div class="pay-container">
      ...
      <div class="checkout-steps">
        ...
        <div class="submit">
          <a class="btn" @click="toQRCode">立即支付</a>
        </div>
        <div class="otherpay">
          <div class="step-tit">
            <h5>其他支付方式</h5>
...
export default {
  ...
  methods: {
    ...
    async toQRCode() {
      //生成二维码
      let qrText = this.payInfo.codeUrl;
      const QRcodeImgUrl = await this.generateQR(qrText);
      // console.log(QRcodeImgUrl);
      this.$alert(`<img src="${QRcodeImgUrl}"/>`, "请使用微信支付", {
    ...
    queryPayOrderStatus() {
      if (!this.payStatusTimer) {
        this.payStatusTimer = setInterval(async () => {
          let result = await this.$api.reqPayOrderStatus(this.payInfo.orderId);
          // console.log(result);
          if (result.code === 200) {
            //支付成功
            clearInterval(this.payStatusTimer);
